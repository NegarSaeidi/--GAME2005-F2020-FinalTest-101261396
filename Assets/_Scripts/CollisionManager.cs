...
public class CollisionManager : MonoBehaviour
{
    ...
    public BulletBehaviour[] spheres,cubeBullets;
    ...
    void Update()
    {
        ...
        // Check each sphere against each AABB in the scene
        foreach (var bulletCube in spheres)
        {
            foreach (var cube in cubes)
            {
                if (cube.name != "Player")
                {
                    CheckCubeAABB(bulletCube, cube);
                }
    ...
    {
       // Debug.Log("cube");
        var MinX = s.transform.position.x - (s.radius / 2);
        var MaxX = s.transform.position.x + (s.radius / 2);
        var MinY = s.transform.position.y - (s.radius / 2);
        var MaxY = s.transform.position.y + (s.radius / 2);
        var MinZ = s.transform.position.z - (s.radius / 2);
        var MaxZ = s.transform.position.z + (s.radius / 2);
      
        if ((MinX<= b.max.x && MaxX >= b.min.x) &&
            (MinY <= b.max.y && MaxY >= b.min.y) &&
            (MinZ <= b.max.z && MaxZ >= b.min.z))
        {
    ...
    public static void CheckAABBs(CubeBehaviour a, CubeBehaviour b)
    {
            ...
            (a.min.z <= b.max.z && a.max.z >= b.min.z))
        {
           ...
           !(a.gameObject.GetComponent<RigidBody3D>().isplayer)&&
           (b.gameObject.GetComponent<RigidBody3D>().isplayer))
            {
                var kinEnergy = (0.5f) * b.gameObject.GetComponent<RigidBody3D>().mass * Vector3.Magnitude(b.gameObject.GetComponent<RigidBody3D>().velocity) * Vector3.Magnitude(b.gameObject.GetComponent<RigidBody3D>().velocity);

                a.gameObject.GetComponent<RigidBody3D>().speed = kinEnergy / a.gameObject.GetComponent<RigidBody3D>().mass;
                var temp = (a.gameObject.GetComponent<RigidBody3D>().transform.position - b.gameObject.GetComponent<RigidBody3D>().transform.position);
                if (temp.magnitude != 0)
                {
                   
                    a.gameObject.GetComponent<RigidBody3D>().direction.Set(temp.x / temp.magnitude, 0.0f, temp.z / temp.magnitude);
                }
            ...
            if (!a.contacts.Contains(contactB))
            {
                ...
                if (contactB.face == Vector3.down)
                {
                    ...
                    a.isGrounded = true;
                  //  a.gameObject.GetComponent<RigidBody3D>().acceleration.Set(0.0f, 0.0f, 0.0f);
                }
                if ((a.gameObject.GetComponent<RigidBody3D>().bodyType == BodyType.STATIC) &&
          (b.gameObject.GetComponent<RigidBody3D>().isplayer) &&
          !(a.gameObject.GetComponent<RigidBody3D>().isplane))
                {
                   
                  
                      
                        b.gameObject.GetComponent<RigidBody3D>().HitFront = true;
                    if (contactB.face == Vector3.up)
                        b.gameObject.GetComponent<RigidBody3D>().HitTop = true;
                        b.gameObject.GetComponent<RigidBody3D>().platformHeight = a.max.y;
        ...
        else
        {
            ...
            if (a.contacts.Exists(x => x.cube.gameObject.name == b.gameObject.name))
            {
                ...
                if ((a.gameObject.GetComponent<RigidBody3D>().bodyType == BodyType.DYNAMIC)&&
             !(a.gameObject.GetComponent<RigidBody3D>().isplayer)&& !a.isGrounded)
                {
                   a.gameObject.GetComponent<RigidBody3D>().speed = 0;
           ...
           (b.gameObject.GetComponent<RigidBody3D>().isplayer))
                {
                  ...
                  //  a.gameObject.GetComponent<RigidBody3D>().direction = new Vector3(0.0f, 0.0f, 0.0f);
                }

                if ((a.gameObject.GetComponent<RigidBody3D>().bodyType == BodyType.STATIC) &&
                (b.gameObject.GetComponent<RigidBody3D>().isplayer) &&
                !(a.gameObject.GetComponent<RigidBody3D>().isplane) &&
                (b.gameObject.GetComponent<RigidBody3D>().HitFront))
             
                {

                    b.gameObject.GetComponent<RigidBody3D>().HitFront = false;
                    b.gameObject.GetComponent<RigidBody3D>().HitTop = false;
